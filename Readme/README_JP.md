...
### v3.0（およびそれ以前）のMAGI-Cryptからv3.1のMAGI-Crypt V2へアップグレードする方へ

* MAGI-Crypt V2はアルゴリズムが更新されました。データの移行にはCSVエクスポート／インポートをご利用ください（環境のセキュリティにご注意ください。ネットワークの切断を推奨します）。
...
* SHA-256の200万回ハッシュから、SHA-512の1400万回ハッシュへと強化。
* 「メモリハード」を加え、暴力的な攻撃に対抗します。KDF時に >64MBのメモリを割り当て、大量のランダム・非整列な読み書きを行います。
* 注意：バージョン3.1よりMAGI-CryptはV2にアップグレードされました。データの移行にはCSVエクスポート／インポートをご利用ください。
