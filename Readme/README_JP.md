...
*   いくつかのアカウントや機密情報を管理するためにアカウントパスワード管理プログラムを使用したい方。
*   支払いたくない方。
...
*   プログラムを準備したフォルダに置きます。
*   プログラムを開くと、パスワードを入力するためのインタフェースが表示されます。安全で覚えやすいパスワードを考え、2回入力し、「確認」をクリックして開始します。
*   ここで入力したパスワードは、暗号化された「キー・シード」となり、固有の「カタログ」と直接対応します。このカタログは、複数のアカウントを管理することができます。
*   このディレクトリを将来開くには、単にこのパスワードを入力します。1回入力するだけです。
*   同様に、異なるカタログのグループを追加する場合は、異なるパスワードを入力することもできます。
*   パスワードの代わりにファイルを選択することもできます（「ファイルを開く」を使用）。ファイル名は重要ではありませんが、ファイルの内容は一切変更できません。
...
*   別のカタログに切り替えるには、「ログアウト」ボタンを使用します。終了するには、「作業終了」ボタンをクリックします。
...
*   ショートカットパラメータ「NONOTICE」を使用して、ウィンドウ下部のNOTICEを非表示にできます（非表示にする前に、一度読んでください）。
...
*   プログラム起動時に「Secure Desktop」オプションがあります。それを選択して再起動すると、セキュリティデスクトップ機能が有効になります。この機能は、「SECUREDESKTOP」というパラメータを使用してショートカットに設定することもできます。
...
1.  ディレクトリパスワードは、SHA256で200万回ソルトをかけられています。これは、パスワードを3.5桁増やしたのと同等です。
2.  最終アーカイブは、AES256 CBCモード、PKCS7パディング、およびランダムIVで暗号化されます（注1）。
3.  さまざまな長さのランダムデータがアーカイブに追加され、実際のデータとパスワードの長さを混同します。
4.  パスワードとして任意のファイルを使用できます。最大サイズは128MBです。
5.  ファイルの自動ゼロフィル書き換え機能があります（保存/削除時）（注2）。

### **プログラムのセキュリティ技術に関する情報（注3）**
...
5.  Memory Page Lock使用され、機密情報がスワップファイルにスワップされるのを防止します（注4）。
...
1.  IV 乱数値はデータのハッシュ化からではなく、データ変更後にのみ再暗号化される。
2.  特に圧縮が有効になっているなど特殊な機能を持つディスクでは、動作が保証されない。また、SSDの性質上、物理層にもデータが残ってしまうため、完全に安全な状態にするには、ディスク領域の全面書き換え（Wipe Free Spaceで検索）が必要です。
3.  このタイプのセキュリティ対策は、ハッカーやトロイの木馬の侵入を防御するためのものです。管理者権限でより効果的に動作します。**しかし、これらのセキュリティ対策は、何もしないよりはましであり、最も重要なことは、あなたのコンピュータが決してハッキングされないようにすることであることを知るべきです。**
4.  Windowsの「セキュリティポリシー」を有効にする必要があります。
...
*   **Q:** ディレクトリパスワードを忘れた場合、どうすればよいですか？
*   **A:** それは災難です。私には何もできません。だから、ディレクトリパスワードを決して忘れないでください。
