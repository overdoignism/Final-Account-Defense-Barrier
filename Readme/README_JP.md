...
1.  カタログパスワードはSHA256で200万回ハッシュ。これは、パスワードを3.5桁増やしたのと同等です。（注1）
...
5.  Memory Page Lock使用され、機密情報がスワップファイルにスワップされるのを防止します（注5）。
...
1.  これは標準的なソルト処理方法ではなく、レインボーテーブル攻撃に耐性がありません。ブルートフォース攻撃に対してのみ効果的です。これはオープンソースのプログラムであるため、ソルト化手法も公開されているため、ソルト化はレインボーテーブルに対抗することができません。強力で長いパスワードを選ぶことが解決策であり、またはコードを変更して専用のソルト化を追加することもできます。
...
4.  このタイプのセキュリティ対策は、ハッカーやトロイの木馬の侵入を防御するためのものです。管理者権限でより効果的に動作します。**しかし、これらのセキュリティ対策は、何もしないよりはましであり、最も重要なことは、あなたのコンピュータが決してハッキングされないようにすることであることを知るべきです。**
5.  Windowsの「セキュリティポリシー」を有効にする必要があります。
