...
### 給從 v2.0 或更舊版升級到 3.0 的使用者

*   3.0 與舊版的資料不相容，請用 CSV 輸出／輸入來進行轉換（請注意環境安全，建議斷網）。
*   如果欲使用非管理者權限，請使用隨付 No_Admin.bat ，並依照需求修改。

### 給從 v3.0 (含之前) MAGI-Crypt 升級到 3.1 MAGI-Crypt V2 的使用者

*   MAGI-Crypt V2 的算法經過升級，請用 CSV 輸出／輸入來進行轉換（請注意環境安全，建議斷網）。
...
*  從 SHA-256 雜湊 200 萬次，提升到 SHA-512 雜湊 1400 萬次。
*  加入了「記憶體困難」對抗暴力破解，KDF 時必須分配 >64MB 記憶體，並且進行大量隨機未對齊讀寫。
*  注意：在 3.1 版後 MAGI-Crypt 升級為 V2，請用 CSV 導出／導入資訊來進行升級。
